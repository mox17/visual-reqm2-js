...
No data is sent to the server side, all processing happens in the browser. The server is only needed because
of standard browser security mechanisms.
...
The doctype table allows to disable display of certain ones. For example `impl` doctypes rarely have
interesting information, it is their presence that counts, so the user may choose not to clutter the diagram
with such and exclude them.
...
Since the pan-zoom functionality uses mouse-drag, selection of text in nodes requires that either Alt or
Ctrl is pressed down when selecting text.
...
For larger graphs the 'auto-opdate' option
...
show the nature and count of the relation to the other doctype.
...
mode and right-click and "copy image".
...
remove the markup and implement simple versions of list rendering. There is no attempt to reflow text,
except from breaking up lines longer than a 110 characters.
...
as a web storage in the browser, for persistence.
